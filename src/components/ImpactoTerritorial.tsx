...
export default function ImpactoTerritorial() {
  ...
  useEffect(() => {
    fetch('/api/impacto')
      ...
      .catch(err => console.error('Error fetching impacto:', err));
  }, []);
  ...
  return (
    <div className="bg-[#F5F2ED] py-16 px-4 sm:px-6 lg:px-8 space-y-16 max-w-7xl mx-auto">
      ...
      <section className="text-center space-y-6 max-w-3xl mx-auto animate-fade-in">
        <span className="text-[10px] uppercase tracking-widest font-bold text-[#C5A059]">{data.mainSubtitle}</span>
        ...
        <p className="text-sm text-[#2C2C2C] font-sans leading-relaxed" dangerouslySetInnerHTML={{ __html: data.introText }}>
        </p>
      </section>
